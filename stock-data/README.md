...
> 当前保留 9 家 A 股事件数据（分众传媒 002027 / 广信股份 603599 / 华域汽车 600741 / 航民股份 600987 /
> 明泰铝业 601677 / 三角轮胎 601163 / 紫金矿业 601899 / 云铝股份 000807 / 盐湖股份 000792）。
> 2026-09-04 名单收敛时，其余 12 家的明细已随公司一并删除；**Wind 事件数据无法免费重建**，
> 若日后把某只股票重新纳入参考池，需再用 `--codes <代码>` 补抓（消耗配额）。
> 其余无事件数据的公司：详情页 ⑥/⑦ 的“Wind 事件增强分”档两列显示 `-`、⑨ 模块隐藏，
> 基础财报档正常评分与展示。

## 每日更新与自查

股票数据由 GitHub Actions 定时抓取，**默认周一至周六每天 2 个时段**触发
（北京 16:00 主 + 22:00 兜底）。若某天页面数据未刷新（详情页 `updated_at` 或 `data/index.json`
...
2. **本地手动补跑**：
   ...
   python fetch_data.py        # 按 config.py 名单重抓最新财务/行情数据并重建 index.json
   ```

   之后 `git add stock-data/data && git commit && git push`
...
- 当前跟踪 13 只 A 股（2026-09-04 收敛，港股与美股全部移出），**名单以 `scripts/config.py` 的 `DEFAULT_COMPANIES` 为准**，
  历史增删记录见 `git log -- stock-data/scripts/config.py`；覆盖全市场（5000+ 只）需改脚本分批抓取；
